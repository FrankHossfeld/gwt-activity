 ...
 * {@link ActivityManager}, in response to {@link org.gwtproject.place.shared.PlaceChangeEvent}
...
public interface Activity {
  ...
  /**
   * Called when the Activity's widget has been removed from container. All event handlers it
   * registered will have been removed before this method is called.
   */
  void onStop();

  /**
   * Called when the Activity should ready its widget for the user. When the widget is ready
   * (typically after an RPC response has been received), receiver should present it by calling
   * {@link AcceptsOneWidget#setWidget} on the given panel.
   *
   * <p>Any handlers attached to the provided event bus will be de-registered when the activity is
   * stopped, so activities will rarely need to hold on to the {@link
   * org.gwtproject.event.shared.HandlerRegistration HandlerRegistration} instances returned by
